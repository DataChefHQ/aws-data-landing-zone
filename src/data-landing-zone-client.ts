...
export class DataLandingZoneClient {
  ...
  constructor(private scope: Construct, private accountId: string, private accountName: string, private region: string) {
  }
  ...
  private getParameter(parameterName: string, errorMessage: string): string {
    const value = DlzSsmReader.getValue(this.scope, `data-landing-zone-client-${parameterName}`, this.accountId, this.region, parameterName) as string;
    if (!value) {
