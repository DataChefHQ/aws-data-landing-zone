  ...
  *  */
  ...
  constructor(scope: Construct, stackProps: WorkloadAccountProps, private props: DataLandingZoneProps) {
    ...
    for (const connection of this.props.network?.connections.vpcPeering || []) {
      ...
      const sourceAccountNetworks = dlzAccountNetworks.getEntitiesForAddress(connection.source, 'account');
      ...
      const destinationAccountNetworks = dlzAccountNetworks.getEntitiesForAddress(connection.destination, 'account');
  ...
  createPeeringRole (from: DlzAccountNetwork, to: DlzAccountNetwork) {
    ...
    console.log(`Creating VPC Peering role '${peeringRoleName}' between account '${from.dlzAccount.name}' and '${to.dlzAccount.name}'`);
