...
import { DLZ_CUR_DEFAULTS, DlzCurAthenaConfig } from './cur-types';
import { SSM_PARAMETERS_DLZ } from '../../stacks/organization/constants';

export interface DlzCurAthenaResolvedEncryption {
  readonly kind: s3.BucketEncryption;
  readonly key?: kms.IKey;
}

export interface DlzCurAthenaProps {
  readonly accountId: string;
  readonly region: string;
  /** Resolved by the data plane so encryption stays consistent with the CUR bucket. */
  readonly encryption: DlzCurAthenaResolvedEncryption;
  readonly config?: DlzCurAthenaConfig;
}
 ...
 */
export class DlzCurAthena extends Construct {
  ...
  constructor(scope: Construct, id: string, props: DlzCurAthenaProps) {
    super(scope, id);

    const cfg = props.config ?? {};
    const expirationDays = cfg.resultsExpirationDays ?? DLZ_CUR_DEFAULTS.athena.resultsExpirationDays;
    if (expirationDays < 1) {
      throw new Error(
    ...
    const bucketPrefix = cfg.resultsBucketNamePrefix ?? DLZ_CUR_DEFAULTS.athena.resultsBucketNamePrefix;
    const bucketName = `${bucketPrefix}-${props.accountId}-${props.region}`;
    this.resultsBucketName = bucketName;
    this.workgroupName = cfg.workgroupName ?? DLZ_CUR_DEFAULTS.athena.workgroupName;
    ...
    const engineVersion = cfg.engineVersion ?? DLZ_CUR_DEFAULTS.athena.engineVersion;
    const publishMetrics = cfg.publishCloudWatchMetrics ?? DLZ_CUR_DEFAULTS.athena.publishCloudWatchMetrics;
  ...
  private encryptionConfiguration(
    encryption: DlzCurAthenaResolvedEncryption,
