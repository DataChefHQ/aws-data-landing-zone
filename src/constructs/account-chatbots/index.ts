...
export class AccountChatbots {
  public static slackChatBots: Record<
    string,
    chatbot.SlackChannelConfiguration
  > = {};

  public static existsSlackChannel(
    scope: Construct,
    chatbotProps: SlackChannel,
  ): boolean {
  ...
  public static addSlackChannel(
    scope: Construct,
    id: string,
    chatbotProps: SlackChannelConfigurationProps,
  ) {
    const account = Stack.of(scope).account;
    const chatBotId = AccountChatbots.slackBotId(account, chatbotProps);

    if (AccountChatbots.slackChatBots[chatBotId]) {
      throw new Error(
        `Chatbot already exists for account ${account} with id ${chatBotId}`,
      );
    }

    AccountChatbots.slackChatBots[chatBotId] =
      new chatbot.SlackChannelConfiguration(scope, id, chatbotProps);
    return AccountChatbots.slackChatBots[chatBotId];
  }
  ...
  public static findSlackChannel(scope: Construct, chatbotProps: SlackChannel) {
    ...
    if (!AccountChatbots.slackChatBots[chatBotId]) {
      throw new Error(
        `No chatbots found for account ${account} with id ${chatBotId}`,
      );
    }
