...
:::tip
Budgets, Cost Anomaly Detection, CUR, and the FinOps account hardening live under the
[`finOps`](/components/finops) prop. Per-account / per-cost-center budgets have a
dedicated page: [Account Budgets](/components/finops/account-budgets).
:::
