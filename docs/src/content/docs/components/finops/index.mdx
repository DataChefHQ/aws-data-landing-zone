...
| [`finOps.accountTags`](/components/finops/account-tags) | Override the FinOps stack's self-tagging defaults |
...
Mandatory tags (`Owner`, `Project`, `Environment`, `CostCenter`, `Name`) are enforced
by tag policy + SCP and applied app-wide. `CostCenter` populates from
`DLzAccount.costCenter`; `finOps.dataExports` activates them as Cost Allocation Tags
automatically.

`DLzAccount.domain` still sets a `Domain` tag on the account, and it remains an active Cost
Allocation Tag, but it is no longer one of the five enforced keys.

Out of scope: dashboards, chargeback, and optimization recommendations. DLZ
delivers the raw cost data. What you do with it downstream is a separate layer.

## Extras (opt-in, not deployed by DLZ)

| Extra | What it is |
|---|---|
| [CUDOS / CID dashboards](/components/finops/cid-dashboards) | AWS Cloud Intelligence Dashboards on top of the DLZ Data Exports. Documents how to deploy them and what they cost. |
...
| Guardrails only | `finOps.budgets`, `finOps.accountBudgets`, `finOps.costAnomalyDetection` (any combination) | Budget alerts and anomaly monitors in the management account; no FinOps account needed |
