...
:::note
AWS Organizations limits each target to **10 attached SCPs**, with a maximum body size of **10,240 characters per policy**
(whitespace is counted when the policy is deployed via CloudFormation; inherited SCPs don't count toward the per-target limit).
...
- **The AWS Config tag rule.** Detective only. It flags non-compliant resources after the fact, across every recorded resource type, and alerts their owner — see [Tag Compliance Alerts](/components/account-management/tag-compliance-alerts).
...
- **Body size.** The rendered policy grows with `actions` × `tagKeys`, and AWS counts whitespace when the policy is deployed via CloudFormation. DLZ validates each SCP body against the 10,240-character limit at synth (see [Validation](#validation)) and does not auto-split. **All four action sets in one SCP do not fit** (~10.7 KB with `exemptAwsServiceCalls`), so split them across two SCPs — see the caution below.
- **Value enforcement stays with the tag policy.** This preset checks presence only, as does the AWS Config rule. Values belong to one owner, and that owner is the tag policy.
...
:::caution[Split the full set across two SCPs]
An SCP can hold **at most 10,240 characters**, and AWS counts the whitespace when the policy is
deployed through CloudFormation (only the console strips it). Spreading **all four action sets into
one SCP is ~10.7 KB — over the limit**, so the deploy fails with *"exceeded the maximum policy
size"*. DLZ catches this at synth and tells you to split.

Put the actions in **two** standalone SCPs instead — for example **CORE + INFRA + IAM** in one and
**DATA_PLATFORM** in the other. A target can have up to 10 attached SCPs and inherited SCPs don't
count, so two on one OU is safe.

<DualCode>
  <Fragment slot="ts">
    ```ts
    const Preset = ScpDenyResourceCreationWithoutStandardTags;
    const tagOnCreateScps = [
      { nameSuffix: 'tag-on-create-core', statements: Preset.statements([
        ...Preset.CORE_TAG_ON_CREATE_ACTIONS,
        ...Preset.INFRA_TAG_ON_CREATE_ACTIONS,
        ...Preset.IAM_TAG_ON_CREATE_ACTIONS,
      ], undefined, { exemptAwsServiceCalls: true }) },
      { nameSuffix: 'tag-on-create-data', statements: Preset.statements(
        Preset.DATA_PLATFORM_TAG_ON_CREATE_ACTIONS, undefined, { exemptAwsServiceCalls: true }) },
    ];
    // attach on the OU: workloads: { standaloneScps: tagOnCreateScps, ... }
    ```
  </Fragment>
  <Fragment slot="python">
    ```python
    ...
    tag_on_create_scps = [
        dlz.DlzStandaloneScp(name_suffix="tag-on-create-core", statements=preset.statements([
            *preset.CORE_TAG_ON_CREATE_ACTIONS,
            *preset.INFRA_TAG_ON_CREATE_ACTIONS,
            *preset.IAM_TAG_ON_CREATE_ACTIONS,
        ], exempt_aws_service_calls=True)),
        dlz.DlzStandaloneScp(name_suffix="tag-on-create-data", statements=preset.statements(
            preset.DATA_PLATFORM_TAG_ON_CREATE_ACTIONS, exempt_aws_service_calls=True)),
    ]
    # attach on the OU: workloads=dlz.OrgOuWorkloads(standalone_scps=tag_on_create_scps, ...)
    ```
  </Fragment>
</DualCode>
:::
...
By default it requires the five canonical mandatory tag keys (`Owner`, `Project`, `Environment`, `CostCenter`, `Name`) — the same set `mandatoryTags` enforces, so prevent and detect never disagree. Pass a second argument to require a different set, e.g. when you also gate `additionalMandatoryTags`.
...
- more than 10 SCPs would be attached to the account
- the policy body exceeds 10,240 characters (measured the way AWS counts it — whitespace included)
